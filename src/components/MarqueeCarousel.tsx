...
export default function MarqueeCarousel({ projects }: MarqueeCarouselProps) {
  ...
  const progress = selectedProject ? Math.min((selectedProject.raised / selectedProject.goal) * 100, 100) : 0;
  ...
  return (
    ...
    >
      {/* Carousel */}
      <div className="overflow-hidden mb-12 w-full" ref={emblaRef} style={{ touchAction: 'pan-y pinch-zoom' }}>
              ...
              <div 
                ...
                style={{
                  transform: isSelected 
                    ...
                    : `scale(0.85) translateY(${offset * 5}px) rotateY(${offset > 0 ? -5 : 5}deg)`,
                  opacity: isSelected ? 1 : Math.max(0.3, 1 - offset * 0.2),
                  filter: isSelected ? 'blur(0px)' : `blur(${Math.min(offset * 0.5, 2)}px)`,
      ...
      {selectedProject && (
        <div className="max-w-2xl mx-auto text-center space-y-6 animate-fade-in">
          <div className="animate-slide-up">
            <h2 className="text-h1 mb-2 leading-tight">{selectedProject.title}</h2>
            <p className="text-subtitle">by {selectedProject.author}</p>
          </div>

          {/* Funding Progress */}
          <div className="glass-panel p-6 max-w-md mx-auto animate-slide-up" style={{ animationDelay: '100ms' }}>
            <div className="flex justify-between items-end mb-3">
              <div>
                <span className="text-3xl font-bold text-primary">${selectedProject.raised.toLocaleString()}</span>
                <span className="text-sm text-muted ml-2">of ${selectedProject.goal.toLocaleString()}</span>
              </div>
